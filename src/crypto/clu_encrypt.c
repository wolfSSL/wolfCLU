        ...
        char* in, char* out, byte* iv, int block, int ivCheck, int inputHex)
{
    ...
    while (length > 0) {
            ...
            if (feof(inFile)) {
                ...
                if (inputHex == 1) {
                    hexRet = wolfCLU_hexToBin(inputString, &input,
                                                ...
                                                NULL, NULL, NULL);
                     if (hexRet != 0) {
                        WOLFCLU_LOG(WOLFCLU_L0, "failed during conversion of input,"
                            " ret = %d", hexRet);
                        return hexRet;
                    }
                }/* end hex or ascii */
        ...
        if (alg == WOLFCLU_CAMELLIA128CBC || alg == WOLFCLU_CAMELLIA192CBC ||
            ...
            if (ret != 0) {
                WOLFCLU_LOG(WOLFCLU_L0, "CamelliaSetKey failed.");
                wolfCLU_freeBins(input, output, NULL, NULL, NULL);
                return ret;
            }
            if (XSTRNCMP(mode, "cbc", 3) == 0) {
                ...
                WOLFCLU_LOG(WOLFCLU_L0, "Incompatible mode while using Camellia.");
                wolfCLU_freeBins(input, output, NULL, NULL, NULL);
                return FATAL_ERROR;
            }
        }
