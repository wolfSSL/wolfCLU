...
int wolfCLU_PKCS12(int argc, char** argv)
{
    ...
    while ((option = wolfCLU_GetOpt(argc, argv, "",
                   pkcs12_options, &longIndex )) != END_OF_ARGS) {
        switch (option) {
        ...
        if (bufSz > 0) {
            ...
            if (buf == NULL) {
                ...
                if (wolfSSL_BIO_read(bioIn, buf, bufSz) != bufSz) {
                    ...
                    if (pkcs12 == NULL) {
                        wolfCLU_LogError("Error allocating pkcs12 struct");
                        ret = WOLFCLU_FATAL_ERROR;
                    }
                    else if (wc_d2i_PKCS12(buf, bufSz, pkcs12) < 0) {
        ...
        if (useDES) {
            ...
            if (hasPassOut) {
                wolfCLU_Log(WOLFCLU_L0,
                        "Warning: -passout ignored because -nodes was set; "
                        "key will be written unencrypted");
            }
