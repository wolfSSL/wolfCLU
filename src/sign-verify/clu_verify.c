                             ...
                             int inForm)
{
    ...
    switch(keyType) {
        case RSA_SIG_VER:
            ret = wolfCLU_verify_signature_rsa(data, out, fSz, keyPath,
                    pubIn, inForm);
            break;

        case ECC_SIG_VER:
            if (wolfCLU_ReadFileToBuffer(hashFile, WOLFCLU_MAX_FILE_SIZE,
                    &hash, &hSz) != WOLFCLU_SUCCESS) {
        ...
        case DILITHIUM_SIG_VER:
            ...
            ret = wolfCLU_verify_signature_dilithium(data, fSz, hash, hSz,
                    keyPath, inForm);
            break;
#endif
        ...
        case XMSSMT_SIG_VER:
            ...
            ret = wolfCLU_verify_signature_xmssmt(data, fSz, hash, hSz,
                    keyPath);
            break;
#endif
        default:
...
int wolfCLU_verify_signature_rsa(byte* sig, char* out, int sigSz, char* keyPath,
    ...
    if (ret == 0) {
        ret = (wolfCLU_ReadFileToBuffer(keyPath,
                WOLFCLU_KEY_FILE_CAP(RSA_MAX_SIZE / 8 * 16),
    ...
    if (inForm == PEM_FORM && ret == 0) {
        ret = wolfCLU_KeyPemToDerFallback_ex(&keyBuf, &keyFileSz, pubIn);
    }
                                 ...
                                 char* keyPath, int pubIn, int inForm)
{
    ...
    if (ret == 0) {
        ret = (wolfCLU_ReadFileToBuffer(keyPath,
                WOLFCLU_KEY_FILE_CAP(MAX_ECC_BITS_NEEDED / 8 * 16),
                &keyBuf, &keyFileSz) == WOLFCLU_SUCCESS) ? 0 :
                WOLFCLU_FATAL_ERROR;
    }
...
int wolfCLU_verify_signature_ed25519(byte* sig, int sigSz,
    ...
    if (ret == 0) {
        ret = (wolfCLU_ReadFileToBuffer(keyPath,
                WOLFCLU_KEY_FILE_CAP(ED25519_PRV_KEY_SIZE * 16),
                &keyBuf, &keyFileSz) == WOLFCLU_SUCCESS) ? 0 :
                WOLFCLU_FATAL_ERROR;
    }
                    ...
                    word32 msgLen, char* keyPath, int inForm)
{
    ...
    if (wolfCLU_ReadFileToBuffer(keyPath, (long)WOLFCLU_MAX_PQ_KEY_PEM_SIZE,
    ...
    if (inForm == PEM_FORM) {
        ret = wolfCLU_KeyPemToDerFallback_ex(&keyBuf, &keyFileSz, 1);
        if (ret != 0) {
            XFREE(keyBuf, HEAP_HINT, DYNAMIC_TYPE_TMP_BUFFER);
            wc_dilithium_free(key);
        #ifdef WOLFSSL_SMALL_STACK
                                  ...
                                  byte* msg, int msgLen, char* pubKey)
{
    ...
    /* open and read public key */
    if (ret == 0) {
        if (wolfCLU_ReadFileToBuffer(pubKey, (long)WOLFCLU_MAX_PQ_KEY_PEM_SIZE,
                &keyBuf, &keyFileSz) != WOLFCLU_SUCCESS) {
            ret = WOLFCLU_FATAL_ERROR;
        }
        else if (keyFileSz < (int)XMSS_OID_LEN) {
            /* The OID is read straight off the front of the file below. */
            wolfCLU_LogError("XMSS public key file is too small to hold an "
                             "OID");
            ret = WOLFCLU_FATAL_ERROR;
        }
    }
                                    ...
                                    byte* msg, int msgLen, char* pubKey)
{
    ...
    if (ret == 0) {
        if (wolfCLU_ReadFileToBuffer(pubKey, (long)WOLFCLU_MAX_PQ_KEY_PEM_SIZE,
            ...
            /* The OID is read straight off the front of the file below. */
            wolfCLU_LogError("XMSS^MT public key file is too small to hold "
                             "an OID");
            ret = WOLFCLU_FATAL_ERROR;
        }
    }
