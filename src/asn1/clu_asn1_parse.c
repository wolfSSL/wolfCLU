...
static Asn1PrintOptions PrintOpts = {0};
...
/* === Functions for decoding and preparing raw data === */
...
int wolfCLU_Asn1Parse(WOLFCLU_ASN1_PARSE_OPTIONS* po)
{
    ...
    if (ret == WOLFCLU_SUCCESS && po->oidFile != NULL) {
    ...
    if (ret == WOLFCLU_SUCCESS) {
        ret = wc_Asn1_Init(&asn1) == 0 ? WOLFCLU_SUCCESS : WOLFCLU_FATAL_ERROR;
    }
    ...
    if (ret == WOLFCLU_SUCCESS) {
        ret = wolfCLU_Asn1Write(&asn1, po, inputFileBuffer, inputFileLen);
    }
