...
int wolfCLU_Rand(int argc, char** argv)
{
    ...
    if (XSTRNCMP("-h", argv[argc-1], 2) == 0) {
        ...
        if (size <= 0) {
            WOLFCLU_LOG(WOLFCLU_L0, "unable to convert %s to a number",
                    argv[argc-1]);
    ...
    if (ret == WOLFCLU_SUCCESS) {
        WC_RNG rng;
        if (wc_InitRng(&rng) != 0) {
            WOLFCLU_LOG(WOLFCLU_L0, "unable to initialize RNG");
            ret = WOLFCLU_FATAL_ERROR;
        }
        else {
            if (wc_RNG_GenerateBlock(&rng, buf, size) != 0) {
                WOLFCLU_LOG(WOLFCLU_L0, "unable to generate RNG block");
                ret = WOLFCLU_FATAL_ERROR;
            }
            wc_FreeRng(&rng);
        }
    ...
    if (ret == WOLFCLU_SUCCESS && useBase64) {
        ...
        if (Base64_Encode(buf, size, NULL, &base64Sz) != LENGTH_ONLY_E) {
            WOLFCLU_LOG(WOLFCLU_L0, "error getting size for base64");
            ret = WOLFCLU_FATAL_ERROR;
        }
        ...
        if (Base64_Encode(buf, size, base64, &base64Sz) != 0) {
            WOLFCLU_LOG(WOLFCLU_L0, "error base64 encoding");
            ret = WOLFCLU_FATAL_ERROR;
        }
    ...
    if (ret == WOLFCLU_SUCCESS) {
        if (wolfSSL_BIO_write(bioOut, buf, size) != size) {
            WOLFCLU_LOG(WOLFCLU_L0, "error writing out RNG data");
            ret = WOLFCLU_FATAL_ERROR;
        }
    }
    ...
    return ret;
#else
    WOLFCLU_LOG(WOLFCLU_L0, "Recompile wolfSSL with RNG support");
