 ...
 *
 * ML-DSA native wolfcrypt implementation for wolfCLU CA sign/verify
 * operations.
 */
...
int wolfCLU_FreeMLDSAKeyHeap(MlDsaKey** key)
{
    ...
    wc_ForceZero(*key, (unsigned int)sizeof(MlDsaKey));
        ...
        MlDsaKey* caKey)
{
    ...
    if (keyRaw != NULL) {
        wc_ForceZero(keyRaw, DILITHIUM_MAX_PUB_KEY_SIZE);
    }
    if (certRaw != NULL) {
        wc_ForceZero(certRaw, DILITHIUM_MAX_PUB_KEY_SIZE);
    }
    if (pubKey != NULL) {
        wc_ForceZero(pubKey, sizeof(*pubKey));
    }
...
/* Convert DER to a freshly allocated PEM buffer. Caller owns *outBuf on
 * success (free with wc_ForceZero + XFREE, DYNAMIC_TYPE_TMP_BUFFER).
 * Returns WOLFCLU_SUCCESS or an error code. */
static int wolfCLU_MLDSADerToPemBuf(const byte* der, int derSz, int pemType,
        byte** outBuf, int* outBufSz)
{
    ...
    if (pemSz <= 0 ||
            wolfCLU_MLDSACheckPemOutSz(pemSz, derSz) != WOLFCLU_SUCCESS) {
        wolfCLU_LogError("wc_DerToPem size query failed: %d", pemSz);
        return (pemSz < 0) ? pemSz : WOLFCLU_FATAL_ERROR;
    }

    pemBuf = (byte*)XMALLOC((size_t)pemSz, HEAP_HINT, DYNAMIC_TYPE_TMP_BUFFER);
    if (pemBuf == NULL) {
        return MEMORY_E;
    }

    ret = wc_DerToPem(der, (word32)derSz, pemBuf, (word32)pemSz, pemType);
    if (ret <= 0) {
        wolfCLU_LogError("wc_DerToPem failed: %d", ret);
        wc_ForceZero(pemBuf, pemSz);
        ...
        const byte* certBuf, int certDerSz, int pemType)
{
    ...
    ret = wolfCLU_MLDSADerToPemBuf(certBuf, certDerSz, pemType, &pemBuf,
            &pemOutSz);
    if (ret != WOLFCLU_SUCCESS) {
        return ret;
    }
 ...
 * from key. days is only used for the self-signed cert case. Returns
 * WOLFCLU_SUCCESS or error. */
        ...
        WOLFSSL_BIO* bioOut, int noOut)
{
    ...
    if (ret == WOLFCLU_SUCCESS) {
        if (isCSR) {
            ret = wc_MakeCertReq_ex(cert, certBuf, (word32)bufSz, mldsaType,
                    key);
        }
        else {
            ret = wc_MakeCert_ex(cert, certBuf, (word32)bufSz, mldsaType, key,
                    &rng);
        }
        if (ret < 0) {
    ...
    if (ret == WOLFCLU_SUCCESS && !noOut) {
        ret = wolfCLU_MLDSAWriteCertBio(bioOut, outForm, certBuf, derSz,
                isCSR ? CERTREQ_TYPE : CERT_TYPE);
    }

    if (certBuf != NULL) {
        wc_ForceZero(certBuf, bufSz);
        ...
        int policySanitized)
{
    ...
    /* x509's basicConstraints/keyUsage can be attacker-controlled CSR
     * content; refuse to sign unless policySanitized says it's safe. */
    if (!policySanitized) {
     ...
     * We use get_pathLength() to read the in-memory field because it respects
     * config overrides, unlike get_ext_d2i() which re-parses original DER. */
    ...
    if (ret == WOLFCLU_SUCCESS &&
            wolfSSL_X509_get_ext_by_NID(x509, NID_subject_key_identifier,
                -1) >= 0) {
        /* subjWcKey is always non-NULL here: the sole caller
         * (wolfCLU_MLDSACertSign) validates subjKey != NULL before calling
         * this function. */
        if (subjWcKey == NULL ||
...
/* Sign a certificate (X509/CSR) with an ML-DSA CA key.
 * *outData receives a heap buffer (caller must XFREE).
 * Note: x509 dates must be set before calling. x509 must remain allocated
 * for the duration to avoid dangling extension pointers. */
#if defined(WOLFSSL_CERT_GEN)
        ...
        byte** outData, int* outDataSz, int policySanitized)
{
    ...
    bufSz = wolfCLU_MLDSASignBufSz(level);
    {
        /* Buffer must hold subject material and extensions. Size for the larger
         * level if the subject is ML-DSA at a higher level than the CA. */
        const WOLFCLU_MLDSA_PARAMS* subjParams =
                wolfCLU_MLDSAParamsByOid(wolfSSL_X509_get_pubkey_type(x509));
    ...
    if (ret == WOLFCLU_SUCCESS) {
        ret = wolfCLU_X509FillCert(x509, newCert, sigType,
                subjKey, wcKeyType, caKey, mldsaType, caCert,
                policySanitized);
    }

    if (ret == WOLFCLU_SUCCESS && caCert != NULL && caCert == x509) {
        /* ca -selfsign: match MakeMLDSASelfSignedCert. Leave pathLenConstraint
         * as-is from CSR/config to match unconstrained req -x509 semantics. */
    ...
    if (ret == WOLFCLU_SUCCESS && outForm == PEM_FORM) {
        ret = wolfCLU_MLDSADerToPemBuf(certBuf, certSz, CERT_TYPE, &outBuf,
                &outBufSz);
    }
    if (ret == WOLFCLU_SUCCESS) {
        if (outForm == PEM_FORM) {
    ...
    if (outBuf != NULL) {
        wc_ForceZero(outBuf, (unsigned int)outBufSz);
