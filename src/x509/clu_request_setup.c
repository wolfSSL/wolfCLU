...
#endif /* WOLFSSL_CERT_REQ && !WOLFCLU_NO_FILESYSTEM */
...
int wolfCLU_requestSetup(int argc, char** argv)
{
    ...
    if (ret == WOLFCLU_SUCCESS && in != NULL) {
        ret = wolfCLU_LoadKey(in, &keyCtx);
        if (ret == WOLFCLU_SUCCESS) {
            pkey = keyCtx.evp;
            if (pkey != NULL) {
                if (wolfSSL_X509_set_pubkey(x509, pkey) != WOLFSSL_SUCCESS) {
                    ret = WOLFCLU_FATAL_ERROR;
                }
            }
#ifdef WOLFCLU_MLDSA_CERTGEN
            else if (keyCtx.key != NULL) {
                isMLDSA = 1;
                mldsaMemKey = (MlDsaKey*)keyCtx.key;
            }
#endif
        }
    }
    ...
    if (ret == WOLFCLU_SUCCESS && keyType != NULL && keyInfo != NULL &&
...
#ifdef WOLFCLU_MLDSA_CERTGEN
        /*Reject -newkey if -key gave an ML-DSA key. */
        if (isMLDSA) {
            wolfCLU_LogError("Cannot combine -key (ML-DSA) with -newkey; "
                    "provide only one key source");
            ret = WOLFCLU_FATAL_ERROR;
        }
#endif /* WOLFCLU_MLDSA_CERTGEN */

        if (ret == WOLFCLU_SUCCESS && XSTRNCMP("ec", keyType, 2) == 0) {
...
#ifdef WOLFCLU_MLDSA_CERTGEN
        /*!isMLDSA: Guard against -newkey clobbering a provided ML-DSA -key          (since ML-DSA keys leave pkey NULL). */
        if (ret == WOLFCLU_SUCCESS && !isMLDSA &&
            ...
            /* Reject before generating/writing any key material. */
            if (reqIn != NULL) {
                wolfCLU_LogError(
                        "Re-signing an existing ML-DSA CSR/cert is not "
                        "supported");
                ret = WOLFCLU_FATAL_ERROR;
            }

            if (ret == WOLFCLU_SUCCESS &&
                    (levelArg < 0 || levelArg > 255 ||
                     wolfCLU_MLDSALevelToKeyGenParams((byte)levelArg,
                             &mlLevel, &mlKeySz, &mlWithAlg) !=
                             WOLFCLU_SUCCESS)) {
                ret = USER_INPUT_ERROR;
            }
            ...
            if (ret == WOLFCLU_SUCCESS) {
                ...
                if (ret == WOLFCLU_SUCCESS) {
                    ...
                    if (keyOut != NULL) {
                        wolfCLU_Log(WOLFCLU_L0, "ML-DSA key pair written to %s.priv "
                                "and %s.pub", kName, kName);
                        if (passout) {
                        ...
                        /*No -keyout: match the RSA/EC -newkey convention                          (below, ~line 1298) of printing the generated                          private key instead of silently discarding it. */
                        byte* derBuf = (byte*)XMALLOC(mlKeySz, HEAP_HINT,
                                DYNAMIC_TYPE_TMP_BUFFER);
                        if (derBuf == NULL) {
                            ret = MEMORY_E;
                        }
                        else {
                            int derSz = wc_MlDsaKey_PrivateKeyToDer(
                                    mldsaMemKey, derBuf, (word32)mlKeySz);
                            if (derSz < 0) {
                                ret = derSz;
                            }
                            else {
                                byte* pemBuf = NULL;
                                int pemSz = wolfCLU_KeyDerToPem(derBuf, derSz,
                                        &pemBuf, PKCS8_PRIVATEKEY_TYPE,
                                        DYNAMIC_TYPE_TMP_BUFFER);
                                if (pemSz <= 0 || pemBuf == NULL) {
                                    ret = WOLFCLU_FAILURE;
                                }
                                else {
                                    wolfCLU_Log(WOLFCLU_L0, "Warning: ML-DSA "
                                            "private key written unencrypted "
                                            "to stdout (no -keyout given)");
                                    XFWRITE(pemBuf, 1, (size_t)pemSz, stdout);
                                    wc_ForceZero(pemBuf, pemSz);
                                    XFREE(pemBuf, HEAP_HINT,
                                            DYNAMIC_TYPE_TMP_BUFFER);
                                }
                            }
                            wc_ForceZero(derBuf, mlKeySz);
...
#endif /* WOLFCLU_MLDSA_CERTGEN */
        if (ret == WOLFCLU_SUCCESS && XSTRNCMP("rsa", keyType, 3) == 0) {
    ...
    if (ret == WOLFCLU_SUCCESS && isMLDSA &&
            (config != NULL || ext != NULL || addExt != NULL)) {
        wolfCLU_Log(WOLFCLU_L0, "Warning: ML-DSA ignores "
                "-config/-extensions/-addext; only the subject DN%s is "
                "emitted",
                genX509 ? " and basicConstraints (CA:TRUE/FALSE)" : "");
    }
    ...
    if (ret == WOLFCLU_SUCCESS && isMLDSA && (in != NULL || mldsaMemKey != NULL)) {
        if (genX509) {
            ret = wolfCLU_MakeMLDSASelfSignedCert(in, mldsaMemKey, x509, days, outForm,
                    bioOut, noOut);
        }
        else {
            ret = wolfCLU_MakeMLDSACSR(in, mldsaMemKey, x509, outForm, bioOut, noOut);
        }
