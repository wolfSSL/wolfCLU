...
class TestCASelfSign(unittest.TestCase):
    ...
    def test_selfsign_and_altextend_mutually_exclusive(self):
        """-selfsign combined with -altextend must be rejected."""
        out_name = "tmp_selfsign_altextend.pem"
        out = _tmp(out_name)
        self._clean(out)
        ...
        self.assertNotEqual(r.returncode, 0,
                            "-selfsign and -altextend should be rejected "
                            "together")
        self.assertIn("mutually exclusive", r.stdout + r.stderr,
                      "expected a mutual-exclusivity error message")
    ...
    def test_selfsign_is_usable_ca(self):
        """A -selfsign'd cert with no -extensions must default to CA:TRUE         (matching req -x509 semantics) and be usable as its own -CAfile,         the same invariant test_mldsa_ca_selfsign checks for ML-DSA."""
        out_name = "test_ca_selfsign_usable.pem"
        out = _tmp(out_name)
        self._clean(out)
        r = run_wolfssl("ca", "-config", self.conf,
                        "-in", self.csr, "-out", out_name,
        ...
        self.assertIn("CA:TRUE", rt.stdout,
                      "self-signed cert must carry CA:TRUE "
                      "(basicConstraints round-trip regression)")

        rv = run_wolfssl("verify", "-partial_chain", "-CAfile", out, out)
        self.assertEqual(rv.returncode, 0,
                         "self-signed cert must verify against itself: " +
                         rv.stdout + rv.stderr)
...
class TestCACreateAndVerify(unittest.TestCase):
    ...
    def test_ca_sign_rejects_csr_embedded_ca_true(self):
        """Regression test: a CSR with basicConstraints=CA:TRUE must not be         issued as a CA when RSA-signed without an explicit -extensions         override."""
        csr_conf = _tmp("ca_evil_csr.conf")
        evil_csr = _tmp("ca_evil.csr")
        out = _tmp("tmp_evil_issued.pem")
        ...
        # Sign with the default RSA CA (from CA_CONF), no -extensions: the
        # CSR's CA:TRUE must not survive.
        r = run_wolfssl("ca", "-config", self.conf,
                        "-in", evil_csr, "-out", out)
        self.assertEqual(r.returncode, 0, r.stderr)
        ...
        self.assertEqual(rt.returncode, 0, rt.stderr)
        self.assertNotIn("CA:TRUE", rt.stdout + rt.stderr,
                         "CSR-embedded CA:TRUE must not survive RSA CA "
                         "signing without an explicit -extensions override")

    def test_ca_sign_override_ca_false_with_extensions(self):
        """An explicit -extensions config asserting CA:TRUE must successfully
        override the CA:FALSE neutralization."""
        csr_conf = _tmp("ca_ext_override_csr.conf")
        csr = _tmp("ca_ext_override.csr")
        ext_conf = _tmp("ca_ext_override.conf")
        out = _tmp("tmp_ext_override_issued.pem")
        self._clean(csr_conf, csr, ext_conf, out)
        ...
        with open(ext_conf, "w", encoding="utf-8", newline="\n") as f:
            f.write(
                CA_CONF +
                "\n[ v3_ca_override ]\nbasicConstraints = critical,CA:TRUE\n"
            )
        ...
        r = run_wolfssl("req", "-new",
                        ...
                        "-config", csr_conf,
                        "-out", csr)
        self.assertEqual(r.returncode, 0, "CSR creation failed: " + r.stderr)

        r = run_wolfssl("ca", "-config", ext_conf, "-in", csr, "-out", out,
                        "-extensions", "v3_ca_override")
        self.assertEqual(r.returncode, 0, "CA signing failed: " + r.stderr)

        rt = run_wolfssl("x509", "-in", out, "-text", "-noout")
        self.assertEqual(rt.returncode, 0, rt.stderr)
        self.assertIn("CA:TRUE", rt.stdout + rt.stderr,
                      "CA:TRUE override via -extensions failed")
...
class TestCAMLDSA(unittest.TestCase):
    ...
    def test_mldsa_ca_selfsign(self):
        """`ca -selfsign` with an ML-DSA key (caCert == x509 in         wolfCLU_CertSign/wolfCLU_MLDSACertSign)."""
        ...
        r_v = run_wolfssl("verify", "-partial_chain", "-CAfile", out, out)
        self.assertEqual(r_v.returncode, 0, r_v.stderr + r_v.stdout)
    ...
    def test_mldsa_issued_cert_wrong_ca_fails(self):
        """A cert signed by ML-DSA-CA-1 is rejected when CA-2 is the trust         anchor."""
    ...
    def test_mldsa_ca_missing_out_fails(self):
        ...
        self.assertTrue(
            ...
            or "unable to open output file" in msg,
    ...
    def test_mldsa_ca_sign_preserves_ip_san(self):
        ...
        r = run_wolfssl("ca", "-config", self.conf,
                        ...
                        "-cert", self.mldsa_ca_cert,
                        "-keyfile", self.mldsa_priv)
        if "warning: cannot copy subjectaltname" in r.stderr.lower():
            self.skipTest("wolfSSL build missing wc_SetAltNamesFromList; skipping SAN copy")
        self.assertEqual(r.returncode, 0, r.stderr)
    ...
    def test_mldsa_verify_rejects_non_ca_cafile(self):
        """A -CAfile that is an ML-DSA leaf (basicConstraints CA:FALSE) must be         rejected on the ML-DSA verify fast path, mirroring the CA:TRUE         requirement X509_verify_cert enforces for RSA/ECDSA."""
        ...
        import shutil
        shutil.copy2(leaf, leaf + ".copy")
        rv = run_wolfssl("verify", "-CAfile", leaf + ".copy", leaf)
    ...
    def test_mldsa_ca_sign_rejects_csr_embedded_ca_true(self):
        """Regression test: a CSR with basicConstraints=CA:TRUE must not be issued         as a CA when signed by an ML-DSA CA without an explicit -extensions override."""
    ...
    def test_mldsa_verify_partial_chain_intermediate(self):
        """An ML-DSA intermediate CA (CA:TRUE but not self-signed) is rejected         as a -CAfile without -partial_chain and accepted with it."""
