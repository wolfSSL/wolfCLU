...
class TestX509Verify(unittest.TestCase):
    ...
    def test_verify_self_as_ca_fails(self):
        ...
        server-cert.pem is a leaf+root bundle, so isolate just the leaf."""
...
class TestX509VerifyChain(unittest.TestCase):
    ...
    def test_cafile_bundle_skips_non_ca_cert_to_find_root(self):
        ...
        server-cert.pem is a [leaf, root] bundle where the leaf does not
        assert basicConstraints CA:TRUE; the scan must skip that cert
        instead of aborting, so it still reaches the root later in the
        bundle."""
        r = run_wolfssl("verify", "-CAfile",
                        os.path.join(CERTS_DIR, "server-cert.pem"),
                        os.path.join(CERTS_DIR, "server-cert.pem"))
