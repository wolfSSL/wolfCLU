...
class TestX509VerifyMLDSA(unittest.TestCase):
    ...
    def test_verify_self_signed_ok(self):
        ...
        r = run_wolfssl("verify", "-partial_chain", "-CAfile", cert, cert)
        self.assertEqual(r.returncode, 0, r.stderr)
    ...
    def test_verify_bad_cafile_fails(self):
        """A -CAfile that cannot be loaded must fail explicitly, not fall back to self-verification and report a spurious OK."""
    ...
    def test_verify_der_companion_pub(self):
        """wolfCLU_LoadMLDSACompanionPub accepts a DER-format companion."""
