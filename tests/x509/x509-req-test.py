...
class TestReqMLDSACert(unittest.TestCase):
    ...
    def _test_newkey_level(self, level):
        ...
        r2 = run_wolfssl("verify", "-partial_chain", "-CAfile", cert, cert)
    ...
    def test_newkey_without_x509_makes_csr(self):
        """ML-DSA -newkey without -x509 produces a PKCS#10 CSR; with -keyout the         freshly generated key pair is retained for later reuse."""
    ...
    def test_newkey_keyout_roundtrip(self):
        ...
        r2 = run_wolfssl("verify", "-partial_chain", "-CAfile", cert2, cert2)
        self.assertEqual(r2.returncode, 0,
                         "verify of round-trip cert failed: " + r2.stderr)

    def test_newkey_no_keyout_no_x509_cleans_temp(self):
        """-newkey ml-dsa (no -x509, no -keyout) produces a CSR and never         writes the generated key to any file - it is printed to stdout         instead, with a warning, so it isn't silently lost."""
        ...
        with open(csr, encoding="utf-8") as f:
            self.assertIn("BEGIN CERTIFICATE REQUEST", f.read(),
                          "output is not a PKCS#10 certificate request")
        self.assertIn("no -keyout given", r.stdout + r.stderr,
                      "expected a warning that the key was not written to a file")
        self.assertIn("BEGIN PRIVATE KEY", r.stdout,
                      "generated private key should be printed to stdout")
        self.assertFalse(os.path.exists(tmp_priv),
                         "no private key file should ever be written")
        self.assertFalse(os.path.exists(tmp_pub),
                         "no public key file should ever be written")
    ...
    def test_newkey_no_keyout_cleans_temp(self):
        """-newkey ml-dsa without -keyout must not leave temp key files         behind - and must not silently discard the generated key either;         it is printed to stdout with a warning instead."""
    ...
    def test_config_extensions_warns(self):
        ...
        # ML-DSA -x509 ignores -config entirely (see warning above), so no
        # basicConstraints extension is written into the cert at all.
        # should_skip_non_ca_cert now skips a self-signed cert with no
        # basicConstraints by default (legacy pre-RFC-5280 CAs need an
        # explicit opt-in), so -CAfile verify fails unless -legacy_ca (or
        # -partial_chain) is given.
        import shutil
        shutil.copy2(cert, cert + ".copy")
        r2 = run_wolfssl("verify", "-CAfile", cert + ".copy", cert)
        self.assertNotEqual(r2.returncode, 0,
                            "verify should reject a self-signed ML-DSA cert "
                            "with no basicConstraints by default")

        r3 = run_wolfssl("verify", "-CAfile", cert + ".copy", "-legacy_ca", cert)
        self.assertEqual(r3.returncode, 0,
                         "verify should accept a self-signed ML-DSA cert "
                         "with no basicConstraints as a legacy CA when "
                         "-legacy_ca is given: " + r3.stderr)
        self.assertIn("no basicConstraints", r3.stdout + r3.stderr,
                      "expected a legacy-CA warning about missing "
                      "basicConstraints extension")

        r3 = run_wolfssl("verify", "-partial_chain", "-CAfile", cert, cert)
        self.assertEqual(r3.returncode, 0,
                         "partial_chain verify failed for ml-dsa -config "
                         "cert: " + r3.stderr)
    ...
    def test_csr_missing_pub_sibling_fails(self):
        """CSR generation fails when the companion."""
    ...
    def test_csr_corrupt_key_fails(self):
        """CSR generation fails gracefully when -key points to a garbage file,         exercising wolfCLU_LoadMLDSAKey error path on the CSR branch."""
